...
void converterEntrada(char entrada[], int cpf[]){   
    int i, j, tamanho;
    for(i=0; i<11; i++){ //Inicializando todo o vetor com zeros, assim caso algum cpf tenha menos que 11 dígitos, irá completar com zeros na frente.
        cpf[i]=0;
    }
    j=10;
    ...
    for(i=tamanho-1; i>=0; i--){ //Será lido da direita para esquerda.
        ...
        if(j<0){ //Para proteger de entradas com mais de 11 dígitos.
            break;
        }
    }
}
