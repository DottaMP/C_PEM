...
{	int l, c;
	for(l=0; l<linhas; l++){	
	    for(c=0; c<colunas; c++){	
	        original[l][c] = (int)(original[l][c] * 1.2); // Casting (int)=converte para um inteiro, tirando a parte fracionada.
...
{	int l, c;
	for(l=0; l<linhas; l++){	
	    for(c=0; c<colunas; c++){	
	        original[l][c] = (int)(original[l][c] * 0.8); // Casting (int)=converte para um inteiro, tirando a parte fracionada. A conta pode tb / por 1.2
		}
	}
}
...
void moldura(int** original, int linhas, int colunas, int maxValor){
    ...
    for(l=0; l<linhas; l++){	
		for(c=0; c<colunas; c++){	
			if(l<(int)(0.05*linhas) || l>(int)(0.95*linhas) || c<(int)(0.05*colunas) || c>(int)(0.95*colunas))
				// original[l][c] = 0; // --> moldura 100%
				original[l][c] = original[l][c]*0.5; // --> moldura tipo insulfilm
		}
	}
/* abaixo código para cada lado
    //cima
	...
	//Direita
	for(l=(int)(0.05*linhas); l<(int)(0.95*linhas); l++)
	{	for(c=(int)(0.95*colunas); c<(int)(1.00*colunas); c++)
		{	original[l][c] = original[l][c]*0.5;
		}
	}
	
	//baixo
	for(l=(int)(0.95*linhas); l<(int)(1.00*linhas); l++)
	{	for(c=0; c<colunas; c++)
		{	original[l][c] = original[l][c]*0.5;
		}
	}
